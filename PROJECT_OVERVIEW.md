...
- Multiple models: Baselines (Linear, Ridge, Polynomial) to advanced (Random Forest, Gradient Boosting, MLP, XGBoost, LSTM with GPU support).
...
### Model Files (`models/`)
- **`__init__.py`**: Model registry. Imports and registers all models (e.g., `linear_regression`, `random_forest_regressor`, `gradient_boosting_regressor`, `mlp_regressor`, `xgboost_regressor`, `lstm_regressor`). Uses lazy loading for optional deps like XGBoost.
...
- **`train_tree_nn.py`**: Trains tree and neural network models (Random Forest, Gradient Boosting, MLP). Similar to above but for ensemble/neural models.
