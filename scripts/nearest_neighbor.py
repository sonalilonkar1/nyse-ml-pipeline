...
def train_knn(window: int, fold: int, params: Dict) -> None:
    prepare_dirs()
    X_train, y_train, X_test, y_test, X_eval, y_eval = load_fold_data(window, fold)

    logger.info("Starting KNN training for window=%d, fold=%d", window, fold)
    knn = KNeighborsRegressor(**params)
    knn.fit(X_train, y_train)

    test_scores = evaluate_model(knn, X_test, y_test)
    eval_scores = evaluate_model(knn, X_eval, y_eval)
    ...
    model_name = f"knn_window{window}_fold{fold}"
    joblib.dump(knn, OUTPUT_MODELS / f"{model_name}.pkl")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="KNN training (alternative style).")
    parser.add_argument("--window", type=int, required=True)
    parser.add_argument("--fold", type=int, required=True)
    parser.add_argument(
        "--params",
        default="{}",
        help='KNN hyperparameters as JSON string, e.g. \'{"n_neighbors":5,"weights":"distance"}\'',
    )
    args = parser.parse_args()

    hyperparams = parse_hyperparameters(args.params)
    train_knn(args.window, args.fold, hyperparams)
